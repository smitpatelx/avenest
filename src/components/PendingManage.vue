<template>
    <div class="w-full">
            ...
            <tbody v-if="showusers" class="bg-gray-100 text-base text-black flex flex-wrap justify-center items-start w-full overflow-y-scroll custom-scroll" style="height: 70vh;">
                <tr v-for="(data,i) in users" :key="i" class="border-b border-t border-solid border-gray-400 flex flex-wrap justify-center items-center w-full">
                    ...
                    <td class="w-2/12 px-2 py-3 font-normal flex flex-wrap justify-center items-center">
                        ...
                        <button @click="change_status(data.user_id)" class="focus:outline-none m-1 bg-transparent hover:text-black text-gray-600 p-2 flex flex-wrap justify-center items-center w-auto cursor-pointer font-semibold underline text-center">Approve</button>
                    </td>
                </tr>
            </tbody>
            <tbody v-else>
        ...
        <notifications group="pending_man" :classes="notify_color" position="bottom left"/>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        change_status(id){
            var bodyFormData = new FormData();
            bodyFormData.append('user_id', id); 
            bodyFormData.append('case', 'approve_user');
            ...
            axios({
                ...
                this.notify_color = "success vue-notification";
                var tt = "Success Approving User:"+id;
                this.$notify({
                    group: 'pending_man',
                    title: 'Success!',
                    text: tt
